...
A dataset too short-running to be quiet asks for more than one pass over its
samples -- see `datasets.DATASET_REPEATS` -- and gets one row per pass in its
jsonl, for the plot to reduce to the fastest of them.
"""
...
def measurement_path(binary: Path, index: int = 0) -> Path:
    ...
    Snakefile's ``time`` rule writes for a one-off target; later passes number
...
def measure(needed: Mapping[Sample, int], machine_name: str) -> dict[Sample, list[str]]:
    ...
    back: a repeat only buys the plot's minimum anything if the passes are far
    enough apart that one disturbance cannot land in all of them.
    """
...
def write(
    ...
    pass and the first block is what a single pass would have written.
    """
