...
There are two ways to describe the machine itself, and which one to use depends
on whether the machine is one the paper reports.

**Detected (any machine).** Ask the machine to describe itself:

```sh
uv run machine-profile --name your_machine_name_here
```

This writes `machines/<name>.json`, which the harness loads exactly as if the
machine had been written into `machines.py`, so `MACHINE=<name>` then works
everywhere — including the `DATASET_VARIANTS` fallback in the Snakefile, which
derives the variant list from what the hardware can actually run rather than
requiring an entry. Nothing is looked up in a table of CPUs: `march` is
whatever the compiler resolves `-march=native` to, `isa` follows from the CPU
flags, and whether hardware counters are used follows from a probe that runs
them. See [`machines/README.md`](machines/README.md), and note that `peak_f32`
...
`STATIC_MACHINES` in
[`src/autotuner/machines.py`](src/autotuner/machines.py) — family, ISA,
compiler settings, and, when libxsmm variants are supported, the corresponding
`libxsmm_arch` — and populate `TESTSET` and `DATASET_VARIANTS_BY_NAME` in the
Snakefile. These are frozen by hand so their published numbers stay
reproducible, and a detected profile is refused if it would shadow one.
...
A machine is either written into `STATIC_MACHINES` in
`src/autotuner/machines.py` or detected into `machines/<name>.json` by
`uv run machine-profile --name <name>`; see [Setting up a new
machine](#setting-up-a-new-machine). Detection covers everything below apart
from `peak_f32`, so the rest of this section is what to fill in by hand for a
machine the paper reports — and what the detected values mean.
...
`freq` only matters when hardware counters are unavailable: with `papi` in
`libs`, cycles come from `PAPI_TOT_CYC` and the field is never read. `peak_f32`
is not discoverable — the vector width is in the CPU flags but the number of
FMA pipes behind it is not — so a detected profile leaves it at `0`, which the
plots read as "no peak known" and fall back to absolute throughput. It can be
added to the profile at any time afterwards: the timings are cached per kernel
in `build/<machine>/**/time.txt` and `peak` only enters when the rows are
assembled from them, so re-running `make dataset` picks up the new value
