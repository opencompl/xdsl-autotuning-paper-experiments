# RUN: clang-20 -DCROWS=4 -DCCOLS=4 -DINNER=4 -DDTYPE=float -o %t kernels/matmul_rowmaj/test.c %s && %t | filecheck %s
...
//  Therefore, the result address (of matrix C) is passed as the first arg (xw).
