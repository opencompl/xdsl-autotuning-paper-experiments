...
rule asm_outer_s:
    input: "kernels/matmul_rowmaj/generate_avx512_outer_asm.py"
    output: machine_file(kernel='matmul_rowmaj', variant='asm_outer', dtype='f64', ext='S')
    shell:
        """
        python3 {input} --M {wildcards.m} --N {wildcards.n} --K {wildcards.k} \
            --output {output}
...
ASM_OUTER_CHECKPOINT_BASES = expand(
    machine_file(
        kernel="matmul_rowmaj",
        k="64",
        dtype="f64",
    ...
    m=(1, 4, 8, 16),
    n=(2, 3, 4),
    variant=("libxsmm", "compxsmm_kdot", "asm_kdot", "asm_outer"),
)

rule asm_outer_checkpoint:
    input:
