...
if shutil.which("pkg-config") and os.system("pkg-config --exists mkl-static-ilp64-iomp") == 0:
    MKL_CFLAGS = shell("pkg-config --cflags mkl-static-ilp64-iomp", read=True).strip()
    MKL_LIBS   = shell("pkg-config --libs mkl-static-ilp64-iomp", read=True).strip()
else:
    MKL_CFLAGS = ""
    MKL_LIBS = ""
