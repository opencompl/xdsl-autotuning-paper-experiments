...
docker-run:
	@if [ "$$(uname -s)" = "Darwin" ]; then \
		...
		nice -n -15 taskset -c 2 USE_PAPI=1 docker run --platform linux/amd64 --cap-add=SYS_ADMIN --cap-add=PERFMON --security-opt seccomp=unconfined --security-opt apparmor=unconfined --pid=host -v .:/src -ti xdsl-autotuner /src/launch.sh; \
	fi
